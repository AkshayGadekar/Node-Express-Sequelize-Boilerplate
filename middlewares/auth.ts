...
const auth = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!token) throw new ErrorResponse({ message: 'Please pass the bearer token.' }, 401)

    try {
        const decoded = jwt.verify(token, jwt_secret) as jwt.JwtPayload
        const tokenType = decoded.type
        if (tokenType !== 'access_token') return next(new ErrorResponse({ message: 'Invalid token.' }, 401))

        const user_id = decrypt(decoded.data)
        const user = await User.findByPk(user_id, { attributes: ['name', 'email'] })
        if (!user) throw new ErrorResponse({ message: 'Invalid token.' }, 401)
        
        const secret = decoded.secret
        const tokensCount = await Token.count({ where: { user_id, secret } })
        if (tokensCount !== 2) return next(new ErrorResponse({ message: 'Token is no longer in use.' }, 401)); // table must've related access token too, so 2 count, result should be 0 or 2
        ...
        next(new ErrorResponse({ message: 'Not authenticated to access this route.' }, 401))
    }
